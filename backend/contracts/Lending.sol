...
contract Lending is Ownable, ReentrancyGuard {
    ...
    function _userCollateralValueInETH(address user) internal view returns (uint256 totalETH) {
        for (uint256 i = 0; i < collateralList.length; i++) {
            ...
            if (amount > 0 && supportedCollateral[token]) {
    ...
    function _tokenValueInETH(address token, uint256 amount) internal view returns (uint256) {
        if (!supportedCollateral[token] || amount == 0) {
            return 0;
        }
        ...
        (bool tokenSuccess, int256 tokenPrice, uint256 tokenUpdatedAt) = _getPrice(tokenFeed);
        if (!tokenSuccess) return 0;

        // Get ETH price in USD  
        (bool ethSuccess, int256 ethPrice, uint256 ethUpdatedAt) = _getPrice(ethUsdFeed);
        if (!ethSuccess) return 0;

        return _calculateValue(token, amount, tokenPrice, ethPrice, tokenFeed);
    }

    function _getPrice(AggregatorV3Interface feed) internal view returns (bool success, int256 price, uint256 updatedAt) {
        try feed.latestRoundData() returns (uint80, int256 _price, uint256, uint256 _updatedAt, uint80) {
            if (_price > 0 && block.timestamp - _updatedAt <= PRICE_TIMEOUT) {
    ...
    function _calculateValue(address token, uint256 amount, int256 tokenPrice, int256 ethPrice, AggregatorV3Interface tokenFeed) internal view returns (uint256) {
        // Get decimals
        uint8 tokenUsdDecimals = _getDecimals(tokenFeed);
        uint8 ethUsdDecimals = _getDecimals(ethUsdFeed);
        uint8 tokenDecimals = _getTokenDecimals(token);

        // Scale prices to 18 decimals
        uint256 tokenUsdScaled = uint256(tokenPrice) * (10 ** (18 - tokenUsdDecimals));
        uint256 ethUsdScaled = uint256(ethPrice) * (10 ** (18 - ethUsdDecimals));
    ...
    function _getDecimals(AggregatorV3Interface feed) internal view returns (uint8) {
    ...
    function _tokenAmountFromETH(address token, uint256 ethAmount) internal view returns (uint256) {
        ...
        try tokenFeed.latestRoundData() returns (uint80, int256 tokenUsd, uint256, uint256, uint80) {
            if (tokenUsd <= 0) return 0;
            
            try ethUsdFeed.latestRoundData() returns (uint80, int256 ethUsd, uint256, uint256, uint80) {
                if (ethUsd <= 0) return 0;
                
                return _calculateTokenAmountFromETH(token, ethAmount, uint256(tokenUsd), uint256(ethUsd), tokenFeed);
            } catch {
                return 0;
            }
        } catch {
    ...
    function _calculateTokenAmountFromETH(
        ...
        try tokenFeed.decimals() returns (uint8 tokenUsdDecimals) {
            try ethUsdFeed.decimals() returns (uint8 ethUsdDecimals) {
                uint256 tokenUsdScaled = tokenUsd * (10 ** (18 - tokenUsdDecimals));
                uint256 ethUsdScaled = ethUsd * (10 ** (18 - ethUsdDecimals));
                
                try IERC20Metadata(token).decimals() returns (uint8 tokenDecimals) {
                    uint256 tokenAmountScaled = (ethAmount * ethUsdScaled) / tokenUsdScaled;
                    return tokenAmountScaled / (10 ** (18 - tokenDecimals));
                } catch {
                    return (ethAmount * ethUsdScaled) / tokenUsdScaled;
                }
            } catch {
                return 0;
            }
    ...
    function getUserCollateralValueETH(address user) external view returns (uint256) { 
        // This function will now handle the case gracefully when user has no collateral
        return _userCollateralValueInETH(user); 
    }
    
    function getUserBorrowCapacity(address user) external view returns (uint256) { 
        return (_userCollateralValueInETH(user) * LTV_BPS) / 10000; 
    }
